...
pageSize: number = 3; // Number of payments per page
  ...
  constructor(private formBuilder: FormBuilder, private paymentService: PaymentService,private foldersService: FoldersService,private authService:AuthService) { }
  ...
  resetForm(): void {
    ...
    fetchData(): void {
      ...
      if (userCIN) {
        ...
        (data: Payment[]) => {
        const userPayments = data.filter(item => item['candidatCIN '] === userCIN);
