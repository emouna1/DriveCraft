...
@Component({
  selector: 'app-conduct-lessons',
  templateUrl: './conduct-lessons.component.html',
  styleUrl: './conduct-lessons.component.css'
})
export class ConductLessonsComponent implements OnInit, AfterViewInit  {
    ...
    ngOnInit(): void {
        ...
        this.calendarService.getAllLessons().subscribe(
          (data: CodeLessonExam[]) => {
            console.log(data);
    ...
    populateCalendar(data: CodeLessonExam[]): void {
      const userExamEvents = data
        ...
        .map((item: CodeLessonExam) => {
          return {
            title: `${item.taskCategory} Lesson`,
            start: item.date + 'T' + item.startHour,
            end: item.date + 'T' + item.endHour,
            backgroundColor: item.accomplished ? 'rgb(112, 193, 247)' : 'rgb(248, 26, 163)',
            extendedProps: {
              id: item.id,
              date: item.date,
      ...
      if (this.editEventForm.valid) {
        ...
        this.calendarService.edit(updatedEvent.id,updatedEvent).subscribe(
          response => {
            ...
            if (event) {
              event.setProp('title', updatedEvent.taskType === 'lesson' ? 'Lesson' : 'Exam');
